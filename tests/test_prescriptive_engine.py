...
class TestGerarOutputPrescritivo:
    ...
    def test_output_atrasado_alta_confianca(self):
        ...
        # Act
        resultado = gerar_output_prescritivo(y_pred, y_proba, feature_importance, top_n=3)
        
        # Assert
        assert len(resultado) == 1
        assert resultado[0]["previsao"] == "Atrasado"
        assert resultado[0]["probabilidade_atraso"] == 0.85
        assert resultado[0]["confianca"] == "Muito Alta"  # CORRIGIDO: 0.85 >= 0.75
    ...
    def test_output_pontual_moderada_confianca(self):
        ...
        assert resultado[0]["confianca"] == "Alta"  # CORRIGIDO: 1-0.35=0.65 >= 0.60
    ...
    def test_classificacao_confianca(self):
        ...
        # Muito Alta (>= 0.75)
        resultado = gerar_output_prescritivo(np.array([1]), np.array([0.80]), feature_importance)
        assert resultado[0]["confianca"] == "Muito Alta"
        
        # Alta (>= 0.60)
        resultado = gerar_output_prescritivo(np.array([1]), np.array([0.65]), feature_importance)
        assert resultado[0]["confianca"] == "Alta"
        
        # Moderada (>= 0.50)
        resultado = gerar_output_prescritivo(np.array([1]), np.array([0.55]), feature_importance)
        assert resultado[0]["confianca"] == "Moderada"
        
        # Baixa (< 0.50)
        resultado = gerar_output_prescritivo(np.array([1]), np.array([0.45]), feature_importance)
    ...
    def test_multiplas_predicoes(self):
        ...
        # Act
        resultado = gerar_output_prescritivo(y_pred, y_proba, feature_importance, top_n=2)
    ...
    def test_formato_probabilidade(self):
        ...
        # Act
        resultado = gerar_output_prescritivo(y_pred, y_proba, feature_importance)
